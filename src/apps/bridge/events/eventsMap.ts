...
import { archiveFomod } from '~bridge/nodejs/actions/archiveFomod';
...
export const eventsMap = {
    ...
    [GET_ALL_ANIMATIONS]: async (inputPath: string, slalJsonConfig: string, author: string, idPrefix: string) => {
        ...
        const res = await readAllAnimationsFromInputPath(inputPath, prefix, author, idPrefix);
        return res || {};
    },
    ...
    [ARCHIVE_FOMOD]: async ({ modules, ...otherPackConfig }: PackFullConfig, moduleConfig: ModuleSpecificConfig) => {
        const combinedConfig: CombinedConfig = await mergeConfigs(otherPackConfig, moduleConfig);

        return await archiveFomod(combinedConfig)
    },
    [READ_MODULE_ANIM_OBJECTS]: async ({ modules, ...otherPackConfig }: PackFullConfig, moduleConfig: ModuleSpecificConfig, applyObjectsConfig = true) => {
        const combinedConfig: CombinedConfig = await mergeConfigs(otherPackConfig, moduleConfig);
        const slalPrefix = await getSlalPrefix(combinedConfig.module.slalJsonConfig);

        return await readObjects(combinedConfig, slalPrefix, applyObjectsConfig)
    },
}
